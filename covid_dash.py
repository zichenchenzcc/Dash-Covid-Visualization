...
df_agg = df.iloc[:,column_list].groupby(['country']).agg({'new_cases':'sum','new_deaths':'sum'}).reset_index()

df_new = df.iloc[:,column_list1].drop_duplicates().reset_index().drop('index',axis=1)
...
app.layout = html.Div([
    dbc.Container(dbc.Row(dbc.Col(html.P("Worldwide Covid-19 Information",style={'text-align':'center','font-size':'200%','background-color':'powderblue'})))),
...
def display_graphs(n_clicks, div_children):
    new_child = html.Div(
        style={'width': '48%', 'display': 'inline-block', 'outline': 'thin lightgrey solid', 'padding': 10},
        children=[
            dcc.Graph(
                id={
                    'type': 'dynamic-graph',
            ...
            dcc.Dropdown(
                ...
                multi=True,
                value=["United States", "India", "Brazil", "Russia"],
            ),
            dcc.Dropdown(
                id={
